            ...
            <div class="song-turn">
              <div class="song-rollwrap">
                <div :style="{ animationPlayState: player && player.isPlaying ? 'running' : 'paused' }" :class="{'circling': row.id === currentPlayId }" class="song-img">
                  <img :src="`${row.al.picUrl}`" class="img">
                </div>
              </div>
              <div class="song-lgour">
                <div :style="{ animationPlayState: player && player.isPlaying ? 'running' : 'paused' }" :class="{'circling': row.id === currentPlayId }" class="song-light" />
              </div>
            </div>
          </swiper-slide>
        </swiper>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      swiperPlayList: state => state.player.swiperPlayList,
      currentPlayId: state => state.player.currentPlayId
    })
  },
  watch: {
    '$store.state.player.currentPlayId': {
      handler (val) {
        console.log(val, 'id')
        this.setSwiperPlayList(val)
      }
    },
    '$store.state.player.swiperPlayList': {
      handler (val) {
        this.jumpToSlide()
      }
    }
  ...
  mounted () {
    ...
    if (this.$store.state.player.playList.length === 0) {
      ...
      this.$store.commit('player/updatePlayList', [this.songDetail])
    } else {
      this.setSwiperPlayList()
    }
  },
  methods: {
    jumpToSlide () {
    ...
    setSwiperPlayList (val) {
      let currentId = val || window.$nuxt.$store.state.player.currentPlayId
      let playlist = window.$nuxt.$store.state.player.playList
      let playlistLen = playlist.length - 1
      let currentIdIndex = playlist.findIndex(items => items.id === currentId)
      if (currentIdIndex === 0) {
        if (playlist.length <= 1) {
          this.$store.commit('player/UpdateSwiperPlayList', [playlist[0]])
        } else {
          this.$store.commit('player/UpdateSwiperPlayList', [playlist[0], playlist[1]])
        }
      } else if (currentIdIndex > 0 && currentIdIndex < playlistLen) {
        this.$store.commit('player/UpdateSwiperPlayList', [playlist[currentIdIndex - 1], playlist[currentIdIndex], playlist[currentIdIndex + 1]])
      } else if (currentIdIndex === playlistLen) {
        this.$store.commit('player/UpdateSwiperPlayList', [playlist[playlistLen - 1], playlist[playlistLen]])
      }
    },
    rangeTouchStart (e) {
      this.touching = true
